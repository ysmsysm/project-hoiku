...
export async function loadSharedSettingsWithClient(
  ...
  const itemTemplates = (itemTemplatesResult.data ??
    []) as unknown as SharedSettingsItemTemplateRow[];
  const activeItemTemplateIds = new Set(
    itemTemplates.map((itemTemplate) => itemTemplate.id),
  );
  const activeItemTemplateWeekdays = (
    (weekdaysResult.data ?? []) as SharedSettingsWeekdayRow[]
  ).filter((weekday) => activeItemTemplateIds.has(weekday.item_template_id));

  return toLoadResult(
    mapSharedSettingsRowsToAppData({
      children: (childrenResult.data ?? []) as SharedSettingsChildRow[],
      itemTemplates,
      itemTemplateWeekdays: activeItemTemplateWeekdays,
    }),
  );
}
