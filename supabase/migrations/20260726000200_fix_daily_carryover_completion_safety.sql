...
declare
  ...
  destination_session_prepared_at timestamptz;
...
begin
  ...
  into
    ...
    destination_session_prepared_at
  ...
  if destination_session_prepared_at is not null then
    return pg_catalog.jsonb_build_object(
      'status', 'invalid_state',
      'created_count', 0,
      'updated_count', 0,
  ...
  select
    ...
    source_items.required_quantity,
    source_items.unit,
  ...
  with spot_groups as (
    select
      claimed.item_template_id,
      pg_catalog.max(claimed.required_quantity) as source_required_quantity,
  ...
  updated_spot as (
    ...
    set
      ...
      required_quantity = greatest(
        destination_items.required_quantity,
        spot_groups.source_required_quantity
      ),
      due_date = (
    ...
    where destination_items.family_id = p_family_id
      ...
      and (
        ...
        or destination_items.required_quantity is distinct from greatest(
          destination_items.required_quantity,
          spot_groups.source_required_quantity
        )
  ...
  already_satisfied_spot as (
    ...
    where not exists (
      ...
      and destination_items.carried_from_daily_item_id is not distinct from
        spot_groups.carried_from_daily_item_id
      and destination_items.required_quantity is not distinct from greatest(
        destination_items.required_quantity,
        spot_groups.source_required_quantity
      )
  ...
  updated_rough as (
    ...
    set
      ...
      required_quantity = greatest(
        destination_items.required_quantity,
        rough_groups.source_required_quantity
      ),
      version = destination_items.version + 1
    from rough_groups
    where destination_items.family_id = p_family_id
