...
comment on function public.ensure_daily_session(uuid, uuid, date) is
  'Creates one daily session with its initial active template snapshots, while existing sessions remain unchanged and carryovers stay a separate operation.';
